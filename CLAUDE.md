...
scenarios, plus a standalone **Human-Feedback-in-the-Loop (HFIL)** stage
(`marag feedback user-stories|test-scenarios`) that appends *additional*,
document-grounded stories/scenarios to an existing artifact. Test cases,
execution, report, and the full re-discover-with-feedback command are
**planned, not built**.

**HFIL stage (add-only v1):** a reviewer comment is anchored to a requirement
(user-story stage) or story (test-scenario stage), gated by an LLM against
closed-world retrieved chunk evidence, then generated by the *existing*
generation agents (via an optional `reviewer_directive` prompt param — the only
change to those agents). Outcomes are either new `origin="feedback"` records
merged into the artifact in place (atomic rewrite) or a structured **decline**
(destructive intent / no anchor / ambiguous anchor / gate rejected / duplicate).
Both outcomes are recorded in the `feedback_events` ledger; declines exit 0.
Idempotency: the deterministic `feedback_id` (no timestamp/uuid) + `on conflict
do nothing` makes replaying the identical comment a clean no-op. Files:
`agents/feedback_agent.py` (gate), `workflows/feedback_graph.py`,
`services/feedback_{intent,anchor,builder}.py`. **This is add-only:** update/
delete requests are declined, not executed. The `feedback_events` ledger is the
planned foundation for the deferred re-discover-with-feedback stage.
...
- Install: `uv sync --dev --extra local-llm` (HF) or add `--extra azure` (Azure OpenAI). Add `--extra mcp` for Claude CLI MCP integration.
...
## No-Docker MCP Automation

- Project-local MCP server: `uv run python -m multi_agentic_graph_rag.mcp.server`.
- Claude config: `.mcp.json` registers `marag-mcp` over stdio; secrets remain in `.env`, not `.mcp.json`.
- Tools wrap existing CLI boundaries: health/start/stop stack, ingest, generate user stories, generate test scenarios, full pipeline, artifact lookup/verification, and run status.
- Windows automation is opt-in via `.env`: `MARAG_POSTGRES_SERVICE_NAME`, `NEO4J_DBMS_HOME`, `NEO4J_JAVA_HOME`, and auto-start flags. It starts only configured services/paths, never Docker, never raw SQL/Cypher, and does not stop shared services unless explicit stop flags are enabled.
- MCP resources expose sanitized config, latest health, latest generated artifacts, and run logs. Passwords, tokens, and DSN passwords must stay masked.
