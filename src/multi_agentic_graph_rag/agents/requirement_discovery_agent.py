...
def _build_prompt(
    ...
    chunk_json = json.dumps(
        {"chunk_id": chunk.chunk.chunk_id, "chunk_text": chunk.text},
        ensure_ascii=False,
        indent=2,
    )
    feedback = ""
    if validation_error:
        feedback = (
            ...
            "the same chunk. Use quotes copied from the normalized chunk_text. If a "
            "long quote is hard to copy exactly, choose a shorter exact quote that "
            "supports the fact. "
            f"Validation error: {validation_error}\n"
        )
    return (
        "You are extracting requirement traceability data from exactly one document chunk.\n"
        "Return exactly one JSON object and no other text. Do not include markdown, "
        "code fences, commentary, XML tags, or hidden reasoning.\n"
        f"{feedback}"
        "Input is one JSON object with chunk_id and normalized chunk_text. The chunk "
        "ID, page, section, and source offsets are owned by Python and must not be "
        "returned.\n"
        "Output root field: facts.\n"
        "Each fact entry must contain: fact_id, fact_text, quote, requirements.\n"
        "Each requirement entry must contain: req_id, req_text, requirement_type, "
        "priority, requirement_key.\n"
        "All requirement fields must be JSON strings. Never return null for req_id, "
        "req_text, requirement_type, priority, or requirement_key.\n"
        "Use temporary fact_id and req_id values only, such as F1 and R1. Python will "
        "replace them with permanent IDs.\n"
        "Analyze the entire input chunk_text for the provided chunk_id, including "
        "CHUNK-07 or CHUNK-0007 when that is the input. Extract complete, meaningful "
        "business requirement sentences that can later be used to create user stories, "
        "scenarios, and test cases.\n"
        "Core rule: preserve the source document text. Do not write your own improved "
        "requirement wording. Do not add meaning, domain nouns, actors, conditions, "
        "limits, purposes, causes, consequences, implementation details, or test details "
        "that are not present in the source text.\n"
        "For each meaningful source sentence, bullet, table row, heading-body pair, or "
        "clause, first decide whether it is relevant. A relevant item may be a requirement, "
        "constraint, business rule, acceptance criterion, non-functional requirement, "
        "scope item, out-of-scope item, capability, or system behavior.\n"
        "fact_text must preserve the smallest meaningful source text. Remove only leading "
        "source identifiers, numbering, bullets, or labels such as BR-SEN-001, AC-001, "
        "FR-001, NFR-001, or section numbers when they are not part of the requirement "
        "meaning. Preserve the remaining source wording.\n"
        "quote must be copied exactly from the normalized chunk_text and must be the "
        "smallest useful text span that supports the fact. Do not return start_char, "
        "end_char, page, section, or chunk_id.\n"
        "req_text must be a complete, meaningful business requirement sentence. It must "
        "never be only an identifier, label, heading, placeholder, or source row code. "
        "It must not contain source identifiers, row codes, bullets, numbering, markdown, "
        "labels, headings, placeholders, or unnecessary symbols. Do not copy source "
        "identifiers such as BR-SEN-001, AC-001, FR-001, or NFR-001 into req_text; "
        "do not copy the source identifier into req_text.\n"
        "Requirement validation flow: if the source text is a valid single requirement "
        "and is grammatically correct, return it as req_text without changing it. If it "
        "is a valid single requirement but is not grammatically correct, make only the "
        "smallest grammar correction required. Do not alter the meaning.\n"
        "If one source text contains multiple requirements in a list or coordinated "
        "sentence, split it into separate requirement records. For each split requirement, "
        "reuse the shared source subject and shared source predicate, then attach exactly "
        "one listed item. Preserve the listed item wording.\n"
        ...
        "planning.', emit: 'The system supports real-time monitoring.', 'The system "
        "supports threshold-based alerts.', 'The system supports rule-based control.', "
        "'The system supports cloud reporting.', and 'The system supports maintenance "
        "planning.'. Do not rewrite 'supports' as 'shall support'.\n"
        "If a relevant fact does not contain a separate derived requirement, return one "
        "requirement whose req_text preserves the fact_text as much as possible. If grammar "
        "is incomplete, add only the minimum words needed to make the sentence valid. Do "
        "not add words that change the meaning.\n"
        "Do not merge explicit BR-*, AC-*, FR-*, or NFR-* rows into summaries. Emit each "
        "explicit row as its own requirement record. Remove the source identifier from "
        "req_text, but preserve the remaining row body as closely as possible.\n"
        "Set requirement_type from the source category when clear. Use one JSON string "
        "such as Functional Requirement, Business Requirement, Acceptance Criteria, "
        "Non-Functional Requirement, Security Requirement, Configuration Requirement, "
        "Validation Requirement, Alerting Requirement, Health Requirement, Data Quality "
        "Requirement, Application Requirement, Offline Requirement, Scope Requirement, "
        "or Out-of-Scope Requirement. If unclear, use Functional Requirement.\n"
        "priority must always be one JSON string: High, Medium, or Low. Use High only "
        "for explicit critical, mandatory, safety, security, reliability, or data-loss "
        "language. Use Low only for explicit optional, future, nice-to-have, or out-of-scope "
        "language. Otherwise use Medium.\n"
        "Set requirement_key to a stable functional identity. It must not be a source "
        "identifier. Exclude revision values such as thresholds, dates, amounts, counts, "
        "and temperatures when possible. requirement_key must not be null.\n"
        ...
        "out-of-scope items, capabilities, or non-functional requirements.\n\n"
