...
def _build_prompt(
    ...
    return (
        ...
        "CHUNK-07 or CHUNK-0007 when that is the input. Go through each meaningful "
        "fact and generate complete, testable business requirement sentences that "
        "can later be used to create user stories, scenarios, and test cases.\n"
        "req_text must be a complete, meaningful business requirement sentence. "
        "It must never be only an identifier, label, heading, placeholder, or source "
        "row code such as BR-SEN-001, AC-001, FR-001, or NFR-001.\n"
        "quote must be copied exactly from the normalized chunk_text and should be the "
        "smallest useful text span that supports the fact. Do not return start_char, "
        "end_char, page, section, or chunk_id.\n"
        "Extract all meaningful facts and requirements in the chunk. Many facts and "
        "many requirements are valid. One fact with one requirement is valid. A fact "
        "with no requirements is valid; return that fact with requirements as an empty "
        "array. Return facts as an empty list only when the chunk has no relevant facts, "
        "requirements, constraints, business rules, acceptance criteria, or "
        "non-functional requirements.\n"
        "Do not merge explicit BR-*, AC-*, FR-*, or NFR-* rows into summaries. Emit "
        "each explicit row as its own requirement record, but rewrite req_text as the "
        "meaningful business requirement body and do not copy the source identifier "
        "into req_text. Preserve separate NFR, business, validation, configuration, "
        "alerting, health, data quality, application, security, and offline records "
        "when they appear separately.\n"
        "Set requirement_key to a stable functional identity, not an original source "
        "identifier, and exclude revision values such as thresholds, dates, amounts, "
        "counts, and temperatures when possible.\n\n"
