...
class PromptRequirementDiscovery(StrEnum):
    ...
    SYS_PROMPT_REQUIREMENT_DISCOVERY = (
        ...
        "Do not return any permanent id fields. Return facts and requirements as lists; after "
        "validation, Python assigns permanent UUIDv7-backed requirement, revision, and evidence "
        "IDs independently of requirement_key.\n\n"
        "Primary task:\n"
        ...
        "One requirement_key represents exactly one atomic semantic requirement. When a source "
        "sentence is split into atomic requirements, give every semantically different split a "
        "distinct stable requirement_key based on its semantic content and source provenance, "
        "never its list position alone.\n"
        "Reuse a requirement_key only when requirement_family, actor, modality, action, object, "
        "condition, polarity, and entity discriminators are compatible. Equivalent observations "
        "of the same atomic requirement may reuse the same key.\n"
        "Preserve a document-assigned identifier only in source_req_id when it occurs in the same "
        "exact quote. Never overload one document identifier as a shared requirement_key for "
    ...
    LEDGER_SECTION_HEADER = (
        ...
        "and evidence are recorded. Reuse the known requirement identity hint only when the "
        "current observation is semantically equivalent.\n"
...
class PromptKnowledgeExtraction(StrEnum):
    ...
    SYS_PROMPT_KNOWLEDGE_EXTRACTION = (
        ...
        "chunk text.\n"
        "subject must exactly match the name of one entry in entities, including any leading "
        "article. Do not add or remove the, a, or an.\n"
        "Exactly one of object_name or object_literal must be non-empty. object_name must "
        "exactly match the name of one entry in entities, including any leading article. "
        "object_literal holds values that "
